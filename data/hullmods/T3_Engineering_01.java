...
public class T3_Engineering_01 extends BaseHullMod {

	private static Map HULL_BONUS = new HashMap();
	static {
		HULL_BONUS.put(HullSize.FRIGATE, 1000f);
		HULL_BONUS.put(HullSize.DESTROYER, 2400f);
		HULL_BONUS.put(HullSize.CRUISER, 3200f);
		HULL_BONUS.put(HullSize.CAPITAL_SHIP, 6000f);
	}
	private static Map FLUX_BONUS = new HashMap();
	static {
		FLUX_BONUS.put(HullSize.FRIGATE, 4300f);
		FLUX_BONUS.put(HullSize.DESTROYER, 7800f);
		FLUX_BONUS.put(HullSize.CRUISER, 13000f);
		FLUX_BONUS.put(HullSize.CAPITAL_SHIP, 17000f);
	}
	private static final float ARC_BONUS = 240f;
	private static final float FLUXD_BONUS = 10f;
	private static final float SHIELD_BONUS_TURN = 100f;
	private static final float SHIELD_BONUS_keep = 0.8f;
	private static final float SHIELD_BONUS_taken = 0.8f;
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		stats.getFluxDissipation().modifyPercent(id, (Float)FLUXD_BONUS);
		...
		stats.getShieldTurnRateMult().modifyPercent(id, (Float)SHIELD_BONUS_TURN);		
	}
